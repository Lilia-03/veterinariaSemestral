...
-- Procedimiento simplificado para debugging
-- Ejecutar en SQL Server Management Studio

-- Primero eliminar el existente
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'CrearUsuarioSimple')
    DROP PROCEDURE CrearUsuarioSimple;
GO

-- Crear versión simplificada
CREATE PROCEDURE CrearUsuarioSimple
    @NombreUsuario NVARCHAR(50),
    @Email NVARCHAR(100),
    @PasswordHash NVARCHAR(255),
    @NombreCompleto NVARCHAR(100),
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Log para debugging
        PRINT 'Iniciando CrearUsuarioSimple';
        PRINT 'NombreUsuario: ' + @NombreUsuario;
        PRINT 'Email: ' + @Email;
        PRINT 'RolID: ' + CAST(@RolID AS VARCHAR);
        
        -- Para clientes, crear registro en tabla Cliente primero
        IF @RolID = 3
        BEGIN
            PRINT 'Creando cliente...';
            
            -- Verificar que tenemos todos los datos del cliente
            IF @CedulaCliente IS NULL OR @Telefono IS NULL OR @Direccion IS NULL
            BEGIN
                RAISERROR('Cliente requiere: cédula, teléfono y dirección', 16, 1);
                RETURN;
            END;
            
            -- Verificar si la cédula ya existe
            IF EXISTS (SELECT 1 FROM Cliente WHERE Cedula = @CedulaCliente)
            BEGIN
                RAISERROR('La cédula ya está registrada', 16, 1);
                RETURN;
            END;
            ...
            -- Insertar cliente
            INSERT INTO Cliente (Cedula, Nombre, Teléfono, Email, Dirección, CantidadDeMascotas)
            VALUES (@CedulaCliente, @NombreCompleto, @Telefono, @Email, @Direccion, 0);
        ...
        DECLARE @NuevoUsuarioID INT = SCOPE_IDENTITY();
        PRINT 'Usuario creado con ID: ' + CAST(@NuevoUsuarioID AS VARCHAR);
...
PRINT 'Procedimiento CrearUsuarioSimple creado exitosamente';

-- Test básico (comentado para evitar ejecución accidental)
/*
EXEC CrearUsuarioSimple 
    @NombreUsuario = 'test123',
